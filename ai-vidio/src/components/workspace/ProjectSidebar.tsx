...
export function ProjectSidebar({ selectedId, onSelect }: ProjectSidebarProps) {
  return (
      ...
      <div className="min-h-0 flex-1 space-y-2 overflow-y-auto p-3">
        {mockProjects.map((project) => {
          ...
          return (
            ...
            >
              <div className={cn("relative h-16 w-24 shrink-0 overflow-hidden rounded-lg", project.thumbnailClass)}>
                <span className="absolute bottom-2 right-2 rounded bg-black/65 px-2 py-0.5 font-mono text-xs text-white">{project.duration}</span>
              </div>
              <div className="min-w-0 pt-1">
                ...
                <p className="mt-1.5 flex items-center gap-2 text-xs text-[#1554ff]">
                  ...
                  {project.mode === "slideshow" ? "图片轮播" : "HTML动画"}
                </p>
                <p className="mt-1.5 text-xs text-slate-600">{project.updatedAt}</p>
              </div>
            </button>
          );
        })}
